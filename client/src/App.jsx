...
function App() {
  ...
  const [userId, setUserId] = useState(null); // State for the current user's ID
  ...
  useEffect(() => {
    ...
    socket.on("userId", (id) => {
      setUserId(id); // Receive and set the user ID from the server
    });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (message.trim()) {
      socket.emit("message", { message, userId }); // Send message with userId
      setMessage("");
    }
  };
  ...
  return (
    <div className="App min-h-screen bg-gray-900 flex flex-col">
      <div className="chat-container flex flex-col flex-1 overflow-hidden">
        ...
        <div className="messages-container flex-1 overflow-y-auto p-4 bg-black">
          <div className="messages w-full flex flex-col">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`message mb-2 p-2 text-white rounded-xl shadow-sm flex items-center ${msg.userId === userId ? 'bg-blue-800 self-end' : 'bg-gray-800 self-start'}`}
              >
                {msg.message}
              </div>
            ))}
